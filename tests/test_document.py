...
def test_attr_value_types():
    """Test that AttrValue type accepts all supported types: str, int, float, bool, None."""
    doc = Document()

    # Test with string attribute
    with doc.tag("div", data_string="hello"):
        doc.text("string")

    # Test with int attribute
    with doc.tag("div", data_count=42):
        doc.text("int")

    # Test with float attribute
    with doc.tag("div", data_value=3.14):
        doc.text("float")

    # Test with bool attribute
    with doc.tag("div", data_enabled=True, data_disabled=False):
        doc.text("bool")

    # Test with None attribute (should be omitted)
    with doc.tag("div", data_none=None):
        doc.text("none")

    # Test text() with different types
    with doc.tag("p"):
        doc.text("string")
        doc.text(123)
        doc.text(45.67)
    ...
    html = doc.render()
    assert 'data-string="hello"' in html
    assert 'data-count="42"' in html
    assert 'data-value="3.14"' in html
    assert 'data-enabled="True"' in html
    assert 'data-disabled="False"' in html
    assert "data-none" not in html
    assert "<p>string12345.67True</p>" in html
